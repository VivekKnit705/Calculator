...
for (item of button) {
    item.addEventListener('click', (e) => {
        ...
        // Here we are check pre==1 or inputText=='=' then we calculate the value ans make the pre==0;
            ...
            updateTextBox = eval(updateTextBox).toString()
            result.value = updateTextBox;
        }
        else if (inputText == 'C') {
            updateTextBox = "";
            ...
            updateTextBox = (x * (Math.PI / 180)).toString();
            ...
            var x = eval(result.value);
            updateTextBox = Math.sqrt(x).toString();
            result.value = updateTextBox;

        }
        else if (inputText == 'log') {
            var x = eval(result.value);
            updateTextBox = Math.log(x).toString();
            result.value = updateTextBox;
        }
        else if (inputText == 'exp') {
            var x = eval(result.value);
            updateTextBox = Math.exp(x).toString();
            result.value = updateTextBox;
        }
        else if (inputText == '^') {
            var x = eval(result.value);
            updateTextBox = Math.pow(x).toString();
            result.value = updateTextBox;
        }
        else {
            updateTextBox += inputText;
